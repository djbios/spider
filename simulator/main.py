...
class Leg:
    def __init__(self):
        self.chain = Chain(name='leg1', links=[
            OriginLink(),
            URDFLink(
                name="hip",
    ...
    def calculate_ik(self, position):
...
class IKLegGUI(QWidget):
    ...
    def initUI(self):
        ...
        self.hip_angle_slider, self.hip_angle_label = self.create_slider_with_label(ANGLE_MIN, ANGLE_MAX, INITIAL_HIP_ANGLE, 'Hip', 1)
        self.knee_angle_slider, self.knee_angle_label = self.create_slider_with_label(ANGLE_MIN, ANGLE_MAX, INITIAL_KNEE_ANGLE, 'Knee', 1)
        self.ankle_angle_slider, self.ankle_angle_label = self.create_slider_with_label(ANGLE_MIN, ANGLE_MAX, INITIAL_ANKLE_ANGLE, 'Ankle', 1)
    ...
    def update_leg(self):
        ...
        self.update_angle_sliders()

    def update_plot(self, position):
        ik = self.leg.calculate_ik(position)
        self.ax.cla()
        self.leg.chain.plot(ik, self.ax, show=False)
    ...
    def update_angle_sliders(self):
        ik = self.leg.calculate_ik([self.x_slider.value(), self.y_slider.value(), self.z_slider.value()])
